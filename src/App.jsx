...
function HtmlOrText({ className, text }) {
...
export default function App() {
  ...
  try {
    deckContext = require.context('../decks', false, /\.json$/);
  } catch (e) {
    deckContext = null;
  }

  const availableDecks = useMemo(() => {
    if (!deckContext) return [{ name: 'default', key: '../deck.json', loader: () => sampleDeck }];
    return deckContext.keys().map((k) => ({ name: k.replace(/^\.\//, ''), key: k, loader: () => deckContext(k) }));
  }, []);

  const [selectedDeckKey, setSelectedDeckKey] = useState(() => (availableDecks[0] && availableDecks[0].key) || '../deck.json');
  const [deck, setDeck] = useState(() => {
    // load initial deck
    try {
      if (deckContext && selectedDeckKey && selectedDeckKey !== '../deck.json') return shuffle(deckContext(selectedDeckKey));
    } catch (e) {}
    return shuffle(sampleDeck);
  }); 
    ...
    return (
      <div className="app layout">
        ...
        <main className="main">
          ...
          <button onClick={() => {
            try {
              const loaded = deckContext && selectedDeckKey ? deckContext(selectedDeckKey) : sampleDeck;
              setDeck(shuffle(loaded));
            } catch (e) {
              setDeck(shuffle(sampleDeck));
            }
          }}>Restart</button>
        </main>
      </div>
